...
- 🧠 **Automatic session checkpoints** — every 3 min, everything discussed is summarized (DeepSeek) + stored verbatim as Mnemosyne memories (`source=checkpoint`/`checkpoint-verbatim`, `[checkpoint-cycle:...]` tag); **Checkpoints tab** renders briefings + verbatim with **auto-refresh every 1 min** and **US-format local timestamps (AM/PM)**; sessions are attributed to their project (hybrid detection: known projects + signals in order of strength + `projeto`/`site` keywords, fallback `[global]`); credentials are masked at display time (`***(senha)***`)
