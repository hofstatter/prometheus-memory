...
- 🌐 **远程多用户 MCP（v0.3）** — 任何城市/国家的开发者可通过 **Tailscale**（私有网络）+ **SSE MCP 服务器**（`:8778`）连接其工具（OpenCode、Claude Code、Codex 等），使用**每个开发者独立的令牌**（仅哈希、可吊销）；每个开发者**只能写入自己的**事件/记忆（服务端强制 `author_id`）——为 CTO/CEO 面板奠定基础。参见 `docs/PLAN_MCP_MULTI_USUARIO_REMOTO.md`
