...
- 🧠 **自动会话检查点** — 每 3 分钟，所有对话内容自动摘要（DeepSeek）并以原文形式存入 Mnemosyne 记忆（`source=checkpoint`/`checkpoint-verbatim`，标签 `[checkpoint-cycle:...]`）；**检查点标签页** 展示简报 + 原文，**每 1 分钟自动刷新**，并使用**美式本地时间格式（AM/PM）**；会话按项目归属（混合检测：已知项目 + 按强度排序的信号 + '项目/网站' 关键词，回退 `[global]`）；凭据在显示时被遮蔽（`***(senha)***`）
