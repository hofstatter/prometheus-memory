...
- 🧠 **Checkpoints automáticos de sessão** — a cada 3 min, tudo que foi conversado vira resumo (DeepSeek) + verbatim na memória do Mnemosyne (`source=checkpoint`/`checkpoint-verbatim`, tag `[checkpoint-cycle:...]`); **aba Checkpoints** renderiza briefings + verbatim com **auto-refresh a cada 1 min** e **horário local em formato US (AM/PM)**; sessões atribuídas ao projeto (detecção híbrida: projetos conhecidos + sinais em ordem de força + keywords 'projeto'/'site', fallback `[global]`); credenciais mascaradas na exibição (`***(senha)***`)
