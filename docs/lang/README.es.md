...
- 🧠 **Checkpoints automáticos de sesión** — cada 3 min, todo lo conversado se resume (DeepSeek) y se guarda verbatim en la memoria de Mnemosyne (`source=checkpoint`/`checkpoint-verbatim`, tag `[checkpoint-cycle:...]`); la **pestaña Checkpoints** renderiza briefings + verbatim con **auto-refresco cada 1 min** y **horario local en formato US (AM/PM)**; las sesiones se atribuyen al proyecto (detección híbrida: proyectos conocidos + señales en orden de fuerza + keywords 'proyecto'/'sitio', fallback `[global]`); las credenciales se enmascaran en la visualización (`***(senha)***`)
